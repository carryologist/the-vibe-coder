...
It works, but the iOS Files → GitHub upload step is friction. You're downloading a zip to a local filesystem, opening another app, navigating to the repo, uploading. The ideal would be direct integration between design tools and deployment platforms, cutting out the file management in between. These are the kinds of workflow seams worth smoothing out.
